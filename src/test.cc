...
TEST(List, Iterator_End_Last) {
  ...
  // EXPECT_EQ(*our_it, *std_it); // вопросы по этому пункту
}
...
TEST(List, Insert_Many_Back) {
  s21::list<int> our_list = {1, 2, 3, 4, 5};
  s21::list<int>::iterator our_it;
  our_list.push_back(7);
  our_list.push_back(8);
  our_list.push_back(9);
  our_it = our_list.end();
  // --our_it; // вот тут вопросы. куда мы ссылаемся
  EXPECT_EQ(*our_it, 9);
  --our_it;
  EXPECT_EQ(*our_it, 8);
  --our_it;
...
TEST(List, Insert_Many_Front) {
  s21::list<int> our_list = {1, 2, 3, 4, 5};
  s21::list<int>::iterator our_it;
  our_list.push_front(7);
  our_list.push_front(8);
  our_list.push_front(9);
  our_it = our_list.begin();
  EXPECT_EQ(*our_it, 9);
  ++our_it;
  EXPECT_EQ(*our_it, 8);
...
TEST(Queue, Pop) {
  ...
  // our_queue_int.pop();
  std_queue_int.pop();
  // std_queue_int.pop(); // тут скорее всего надо переписать end
  EXPECT_EQ(our_queue_int.empty(), std_queue_int.empty());
}
