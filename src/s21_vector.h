...
class Vector{
    public:
        ...
        Vector(std::initializer_list<value_type> const &items); // конструктор списка инициализаторов, создает вектор, инициализированный с помощью std::initializer_list
        vector(const vector &v); // копирование 
        vector(vector &&v); // перемещение
        ~vector(); // деструктор
        vector &operator=(vector &&v); // перегрузка оператора присваивания для движущегося объекта
    private:
};
